...
>
> **This skill requires `scrape-x` ≥ 0.2.0 and is built delegated from day one (decided 2026-07-07).** The build session implements and releases scrape-x v0.2.0 (SCRAPER-FOR-X-PLAN §10a: enriched `--help` with the dead paths marked not-implemented, plus a `scrape-x schema` subcommand) FIRST, then builds this skill against it — so the skill never hand-copies a flag table or a Tweet/User/Media field enumeration at all, unlike facebook-fetch v1 which shipped hand-copied and was retrofitted later. The mechanical surface lives in the installed binary (version-matched, drift-free); the skill keeps only what the CLI can't emit.
>
> **Target OS: macOS only for v1.** Browser login is a headed, by-hand GUI step.
>
> **The defining fact of this skill:** `scrape-x search` and `scrape-x fetch --replies` are **NOT implemented** — they fail fast with exit 1. This holds in the required `scrape-x >= 0.2.0`: v0.2.0 only *marks* them not-implemented in `--help` (SCRAPER-FOR-X-PLAN §10a), it does not implement them. The skill must never route Claude to them. What *works* is `fetch` (a profile's tweets) and `tweet --replies` (a full thread). Getting this routing right is the skill's whole reason to exist beyond `scrape-x --help` (§5).
...
**Not this skill:** anonymous one-off fetch of a single public tweet's text with no login (that can go to `ultra-fetch`); logged-in *Facebook* (that's `facebook-fetch`); **X search** and **replies-to-a-profile's-tweets** (not implemented — §5 trap 1; do not accept these as if they work).
...
└── references/
    └── scrape-x.md          # loaded when running a real fetch / handling an error / first-time setup:
                             #   both login paths + one-time setup, the >= 0.2.0 version check, pointers to
                             #   `scrape-x fetch --help` / `tweet --help` / `scrape-x schema` for the mechanical
                             #   flag/field surface + the usage semantics those can't carry, exhaustive
                             #   exit-code table, cookie-import format details, troubleshooting
```
Same one-reference rationale as facebook-fetch (see FACEBOOK-FETCH-PLAN §2): the deep material shares one trigger — "actually running / erroring / first setup." Re-check the split against drafted length. The reference does NOT carry a hand-copied flag table or field enumeration — that's the delegation (header note; the shipped facebook-fetch reference post-§11-revision is the model to copy).
...
- **Version check (`scrape-x` ≥ 0.2.0 required — header note).** The skill points at `scrape-x fetch --help`/`scrape-x schema` for the mechanical surface; both need 0.2.0+. Confirm with `scrape-x schema`: exit 0 means new enough; a pre-0.2.0 install doesn't recognize `schema` and exits 1 with an "invalid choice" argparse error — an upgrade signal (`uv tool upgrade scraper-for-x`), NOT a login problem, and not exit 2. Use `uv tool upgrade scraper-for-x`, never `uv tool install --upgrade scraper-for-x[...]`: the former upgrades in place preserving whichever extra was installed (Path A's `[browser]` or Path B's base), while the `install --upgrade` form re-resolves from the spec as written and would strip `[browser]` from a Path A user or force it onto a Path B user (SCRAPER-FOR-X-PLAN §10a "Versioning"). Carry this as one reference paragraph, same shape as facebook-fetch's (its E2E re-test proved a cold model diagnoses "upgrade, not login" correctly from that paragraph alone).
...
1. **`search` and `fetch --replies` are DEAD — never route to them.** Both raise `FeatureNotImplementedError` → exit 1 *before any network request*, with a "not yet implemented" message. Reason: X requires a fresh, single-use `x-client-transaction-id` per request for `SearchTimeline`/`UserTweetsAndReplies`, which the harvest-then-replay design can't reproduce. **For a thread, use `scrape-x tweet <id> --replies`** (that works — `TweetDetail` needs no such token). **X search has no working substitute** — if the user asks to search X, say so honestly rather than running a command that will fail. From v0.2.0 the CLI's own `--help` also marks both as not-implemented (SCRAPER-FOR-X-PLAN §10a), so `--help` no longer contradicts this trap — but the skill still carries the routing rule in its body, because the moment of routing (picking which command to run for a user request) happens before anyone reads `--help`.
...
8. **`--since` is best-effort; exit 7 is success-with-nuance — and X differs from facebook-fetch here.** A `--since` run that stops before crossing the date — because it ran out of request budget **or because `--limit` was hit first** — exits 7 with `(requested --since NOT confirmed reached)` on stderr; the partial file is valid. Note the cross-skill trap: `scrape-x`'s `_finish` counts `limit_reached` as an inconclusive `--since` stop (exit 7), whereas `scrape-fb` treats "hit `--limit`, as asked" as a complete success (exit 0). A Claude that just used facebook-fetch will wrongly expect exit 0 when `--limit` and `--since` are combined on X — it's exit 7 unless the date was actually crossed. Either way the saved tweets are usable.
...
| **7** | Partial `--since` (requested but not confirmed reached — stopped on `--limit` or the request budget) | Not an error — partial file is valid. Offer: raise `--limit`, narrow `--since`, or `--wait-on-limit`. |
...
- **A tweet object's fields** — plan-reader ground truth ONLY; the skill does NOT copy this list, it points at `scrape-x schema` (header note): `id` (rest_id, stable dedup key), `url`, `created_at` (ISO `…Z`, may be null), `text` (full text; note-tweet long form preferred; for a retweet this is the *original*'s text), `lang`, `author` (nested `User` or null), `is_reply`, `in_reply_to_id`, `conversation_id`, `reply_count`/`retweet_count`/`quote_count`/`like_count`/`bookmark_count`/`view_count` (often null), `media` (`{kind,url,width,height,alt_text}` — signed/expiring `pbs.twimg.com`/`video.twimg.com` URLs, sensitive), `urls` (expanded, not `t.co`), `hashtags`, `is_note_tweet`, `is_pinned`, `retweeted_tweet`/`quoted_tweet` (nested, and — since the serializer recurses — a quote-of-a-quote can go deeper than one level), `is_restricted`, `captured_at` (always present, when captured — never for dedup). The field-attached *usage* gotchas ship in `scrape-x schema`'s own field descriptions (dedup on `id`; retweet-`text` resolution; media-URL sensitivity; the deeper-than-one-level nesting caveat; `is_restricted`; pinned-out-of-order — SCRAPER-FOR-X-PLAN §10a Part B, mirroring shipped fb's `FIELD_DESCRIPTIONS`), so the skill's reference need not restate them beyond echoing the highest-signal one or two as prose. What the skill genuinely keeps hand-written is the non-field material `schema` can't attach to any single key: the skill-context advice (`--output ./.tmp/x-fetch/…`, never `--no-redact`) and the cross-field flag behavior — `--since`/`--until` skip pinned tweets and never compare a `created_at = None` tweet (§6/§11).
...
- **The routing rule + the dead-paths trap up front** (§5 traps 1–2): profiles → `fetch`, tweets/threads → `tweet --replies`, and `search`/`fetch --replies` don't work (not in the required ≥ 0.2.0 either) — this is the single most important thing the body must get across, because it's where a naive caller wastes a run or promises the user something impossible.
...
**references/scrape-x.md — trigger: running a real read, handling an error, or first-time setup.** Holds both login paths + one-time setup (§3), the ≥ 0.2.0 version check (§3), cookie-import format details, pointers to `scrape-x fetch --help`/`scrape-x tweet --help` (flags) and `scrape-x schema` (field schema) plus the hand-written usage semantics those can't carry (§7), the exhaustive exit-code table across all subcommands, and troubleshooting (exit-4 refresh, soft-lock vs IP change, rate-limit waiting, base-vs-`[browser]` install errors, `status --json` to stdout). Most turns that merely decide *whether*/*which command* never load it.
...
- Version-mismatch reasoning (simulatable, no live account): given `scrape-x schema` exiting 1 with "invalid choice: 'schema'" → skill diagnoses "pre-0.2.0 install, upgrade" and does NOT tell the user to log in (mirrors the fb E2E scenario that validated this exact paragraph).
