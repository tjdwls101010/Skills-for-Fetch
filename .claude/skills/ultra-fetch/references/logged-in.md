...
- **G-interrupted-login.** `uf login <site>` creates the profile directory as soon as the browser opens, before any login happens — if the process is killed or its stdin closes before you press Enter (confirmed live: closed stdin raises a clean `EOFError` and tears the browser down safely, no hang or orphaned process), the profile directory is left behind empty/unauthenticated. `uf doctor`'s profile listing can't tell that apart from a real saved session — both just show up as "last touched N days ago". If `--profile <site>` ever hits a login-wall failure on a profile you thought was already logged in, delete `~/.ultra-fetch/profiles/<site>` and re-run `uf login <site>` from scratch rather than assuming the site or the tool is broken.
