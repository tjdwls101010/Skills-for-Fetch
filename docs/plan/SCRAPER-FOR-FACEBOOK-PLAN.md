...
- **Maintainer exposure (not just the user).** Publishing a public package literally named "scraper-for-facebook" under a real identity ties *you, the author*, to it — PyPI Trusted Publishing binds each release to your named GitHub repo/account (here, the `Skills-for-Fetch` monorepo; the repo name is less on-the-nose than the PyPI package name, but the package name and subdir still carry the exposure). Meta has pursued named authors/operators of public FB/IG scrapers before. Documented enforcement skews toward commercial mass-scrapers and data brokers rather than solo personal-scale projects, so the probability for this project is uncertain — but the exposure is real and lands on the publisher, and this plan names it so the decision is informed. (You chose to keep the name and real identity; the DISCLAIMER records this exposure so the choice stays informed.)
...
**Monorepo.** Both the `web-fetch` skill and this package live in the one repo `github.com/tjdwls101010/Skills-for-Fetch`; the package is developed **and published** from the subdirectory `package/scraper-for-facebook/`. GitHub Actions only runs workflows placed at the **repo root** `.github/workflows/`, so the package's CI and publish workflows sit there (package-named + path-filtered), not inside the subdir:
- `.github/workflows/scraper-for-facebook-ci.yml` — `on: push/pull_request` with `paths: [package/scraper-for-facebook/**]` → ruff + pytest fixtures + build + wheel-install smoke + fixture PII scan. (Path filter so skill-only changes don't trigger it, and vice versa.)
- `.github/workflows/scraper-for-facebook-publish.yml` — `on: push tags scraper-for-facebook-v*` → build **from the subdir** → PyPI Trusted Publishing (§15).

The tree below is rooted at the package subdirectory:

```
package/scraper-for-facebook/        (in the Skills-for-Fetch monorepo; PyPI dist "scraper-for-facebook"; command scrape-fb)
...
│                                    (CI/publish workflows live at the REPO ROOT .github/workflows/ — see the monorepo note above)
...
- **First release uses a PENDING publisher (blocker fix), monorepo-scoped.** For a brand-new name there is no project settings page yet. Before the first tag, create a **pending publisher** at PyPI → Account Settings → Publishing: project name `scraper-for-facebook`, owner `tjdwls101010`, repo **`Skills-for-Fetch`** (the monorepo — *not* a separate `Scraper-for-Facebook` repo), workflow filename **`scraper-for-facebook-publish.yml`**, and the environment name if that workflow uses one. It converts to a normal publisher on first successful upload. (Do not follow "register on the project settings page" — that page doesn't exist pre-publish.)
- **`scraper-for-facebook-publish.yml` (monorepo) must actually authenticate.** It triggers on a **package-scoped tag** (`scraper-for-facebook-v*`, so tagging the skill or anything else never publishes the package), checks out the repo, and **builds from `package/scraper-for-facebook/`** (set the build working-directory / `gh-action-pypi-publish`'s `packages-dir` to the subdir's `dist/`). The publish job sets `permissions: id-token: write` (plus `contents: read`) — without it, `gh-action-pypi-publish` cannot mint the OIDC token. Choose **either no environment on both sides, or the same environment name** on both the workflow and the pending-publisher registration (a mismatch → PyPI rejects the token). **Pin `pypa/gh-action-pypi-publish` to a full commit SHA** (a floating tag is a hijack vector for a package that runs against users' authenticated sessions), and scope the trusted publisher to that named environment. (Skip required-reviewer gates — impractical for a solo maintainer.)
...
1. Scaffold at `package/scraper-for-facebook/` (in the Skills-for-Fetch monorepo): `pyproject.toml` (classifiers, `scrape-fb` entry point), `LICENSE` (MIT), `DISCLAIMER.md`, README skeleton (isolated-install + disclaimer), nested scoped `.gitignore` (with `!tests/fixtures/`), ruff + pre-commit (incl. PII/secret scan). Add the **repo-root** workflows `.github/workflows/scraper-for-facebook-ci.yml` (path-filtered to `package/scraper-for-facebook/**`: ruff + fixtures + build + wheel-install smoke) and `…-publish.yml` (skeleton). **Verify the `scrape-fb` console-script name collides with no installed PyPI/Homebrew command** before committing.
...
9. Packaging polish; finish `scraper-for-facebook-publish.yml` (package tag `scraper-for-facebook-v*`, build from the subdir, `id-token: write`, SHA-pinned action, environment); **revoke+verify the exposed token**; create the **pending publisher** (repo `Skills-for-Fetch`, workflow `scraper-for-facebook-publish.yml`); tag `scraper-for-facebook-v0.1.0`.
10. Skill integration (same monorepo): apply the §16 edits to PLAN.md; the skill installs the *published* `scrape-fb` via `uv tool` (not a local path — so it exercises the real distribution) and calls `scrape-fb fetch`/`doctor`.
