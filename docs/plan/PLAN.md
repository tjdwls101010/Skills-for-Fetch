...
>
> **Facebook is a separate package (separate repo).** The logged-in-Facebook capability is split into a standalone open-source tool, `scraper-for-facebook` (CLI `scrape-fb`), which the skill consumes like `gh`/`yt-dlp`. It is its **own git repo** (`github.com/tjdwls101010/Scraper-for-Facebook`), checked out locally at `package/scraper-for-facebook/` but **git-ignored by this repo** — it is NOT part of Skills-for-Fetch, and it publishes to PyPI from its own repo. Its full spec — hardened against a 6-lens adversarial review — is in [SCRAPER-FOR-FACEBOOK-PLAN.md](SCRAPER-FOR-FACEBOOK-PLAN.md). The skill keeps the generic `--capture-xhr` mechanism (Tier-1, arbitrary SPAs); only FB-specific parsing lives in the package.
