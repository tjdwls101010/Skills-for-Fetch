...
### 10a. v0.2.0 — CLI self-description (shipped 2026-07-07, released to PyPI, real local `uv tool` install upgraded)
...
**Implementation order (small) — completed as planned:** help strings + the `schema` subcommand added to `cli.py`; the descriptions dict (`FIELD_DESCRIPTIONS`) + `schema_fields()`/`json_schema()` added beside `model.py`, anchored on `Post.to_dict()` output keys per the trap noted above; unit tests assert `schema`'s documented field set equals `Post.to_dict()`'s keys — checked both without `--raw` (19 keys) and with `--raw` (20 keys) — **not** `dataclasses.fields(Post)`, plus an assertion that every `to_dict` key has a description entry; the wheel-install smoke test now also runs `schema` in CI (both OS legs green). Released as v0.2.0 (tag + GitHub Release → OIDC PyPI publish, verified live on PyPI and via a fresh-venv install); the skill-side delegation (FACEBOOK-FETCH-PLAN §11) was applied the same session, and the machine's real `uv tool install`ed `scrape-fb` was upgraded from 0.1.0 to 0.2.0 so the skill isn't silently pointed at a stale local binary. Re-validated via 4 fresh E2E subagents (command construction under `--help` delegation, a schema/field question, and two reasoning-only exit-code/version-mismatch scenarios) — all four succeeded using only the skill's own docs, no source reads; one agent caught the local 0.1.0 install for real and correctly diagnosed "upgrade, not login" purely from the reference's guidance, which is exactly the behavior §10a's version-mismatch mitigation was designed to produce.
