...
1. Scaffold the package **as its own git repo** at `package/scraper-for-facebook/` (`git init` there; remote = `github.com/tjdwls101010/Scraper-for-Facebook`; the Skills-for-Fetch root `.gitignore` already lists `package/scraper-for-facebook/` — done in planning). Files: `pyproject.toml` (classifiers, `scrape-fb` entry point), `LICENSE` (MIT), `DISCLAIMER.md`, README skeleton (isolated-install + disclaimer), scoped `.gitignore` (with `!tests/fixtures/`), ruff + pre-commit (incl. PII/secret scan), and this repo's own `.github/workflows/ci.yml` + `publish.yml`. **Verify the `scrape-fb` console-script name collides with no installed PyPI/Homebrew command** before committing.
