...
### 10a. v0.2.0 — CLI self-description (planned; implement next session, BEFORE building x-fetch)

Motivation, same as `scrape-fb` v0.2.0 (SCRAPER-FOR-FACEBOOK-PLAN §10a, shipped 2026-07-07) plus one X-specific problem that makes it more urgent here. Shared part: the shipped v0.1.0 `--help` prints most flags bare — on `fetch`, of its 14 optional flags only `--no-redact` carries help text; the other 13 (`--replies`/`--limit`/`--since`/`--until`/`--by`/`--format`/`--output`/`--wait-on-limit`/`--max-wait`/`--profile`/`--profile-dir`/`--raw`/`-v`) have no description and no default (verified against the real PyPI 0.1.0 install, 2026-07-07) — so a skill that wants to delegate its flag table to `--help` can't yet. X-specific part: **the current `--help` actively misleads.** `search` appears in the top-level subcommand list as a normal working command ("Tweets matching a query / advanced operators.") and `--replies` sits unmarked in `fetch --help`, yet both are dead paths that raise `FeatureNotImplementedError` (the `SearchTimeline`/`UserTweetsAndReplies` ops need a fresh single-use `x-client-transaction-id` per request that harvest-then-replay can't mint — live-verified 2026-07-05, documented in `errors.py`'s `FeatureNotImplementedError` docstring; §17's `G-no-txid` records the inverse, that ordinary reads need no such token). `scrape-fb` never had this problem — every subcommand it advertised worked. Fixing the help is therefore not just enabling delegation; it stops the CLI from lying to direct users too. Minor bump → **v0.2.0**: enriched `--help` + a new `schema` subcommand (both purely additive), plus exactly one small deliberate behavior fix (the dead-path exit-1 hoist, Part A below) — no other behavior changes.

**Sequencing (decided with the user, 2026-07-07): next session implements and releases scrape-x v0.2.0 FIRST, then builds the x-fetch skill in the same session, delegated from day one.** Unlike facebook-fetch — which shipped v1 with hand-copied tables and was retrofitted the day the CLI caught up — x-fetch does not exist yet, so it never carries a hand-copied flag table or field enumeration at all (X-FETCH-PLAN §2/§7/§8 are written against `scrape-x >= 0.2.0` accordingly).

**Scope the drift claim honestly — same caveat as fb.** This de-drifts the *skill's* copy only. The package's own `README.md` and `wiki/CLI-Reference.md` / `wiki/Output-Schema.md` / `wiki/Quick-Start.md` / `wiki/Python-API-Reference.md` hand-maintain the same flag/field surface and are not consumers of `schema`/`--help`. Same two options as fb's §10a: (a) regenerate or CI-diff those pages from `schema`/`--help` output — the real fix; (b) scope every "stops drifting" claim to the skill's copy — the honest minimum. fb took (b) by omission; decide explicitly at implementation, and don't let this section claim a project-wide fix if (b) is taken.

**Part A — enrich `--help`, and make the dead paths honest (in `cli.py`).** Every currently-bare flag gets a `help=` string embedding its human default and CLI-intrinsic semantics. Load-bearing ones: the `search` subcommand's help and `fetch`'s `--replies` must state **"NOT IMPLEMENTED in this version — errors out (exit 1)"** with the one-line why (X requires a fresh single-use `x-client-transaction-id` per `SearchTimeline`/`UserTweetsAndReplies` request, which harvest-then-replay can't mint) and, for `fetch --replies`, the working alternative ("for one tweet's thread, use `scrape-x tweet <id> --replies`"). Pointing at a sibling subcommand is CLI-intrinsic routing, so it belongs in the help; when a later version implements these ops, the help text changes with the binary — which is exactly the version-matched property delegation buys.

**The one deliberate behavior fix (needed to make that help text true).** In shipped v0.1.0 the dead-path help would be a lie for a *logged-out* user: `_cmd_search`/`_cmd_fetch` call `_load_read_client` — which returns exit 2 ("Run: scrape-x login…") when no session exists — **before** `retrieve` ever raises `FeatureNotImplementedError`. So today `scrape-x search q` on a fresh install exits **2** and tells the user to perform a real headed login for a feature that does not exist, and only a *logged-in* user sees the exit-1 the help would promise. v0.2.0 fixes this by hoisting the dead-path rejection to the top of the handler (in `_cmd_search`, and in `_cmd_fetch` guarded by `if args.replies`), *ahead of* `_load_read_client`, so both dead paths exit 1 immediately regardless of login state. This is a real exit-code change (2→1 for the logged-out-hitting-a-dead-path case) and the *only* intentional behavior change in v0.2.0 — called out here rather than hidden under "additive", because it is what lets the `--help` marker be unconditionally accurate and stops the CLI asking anyone to log in for a nonexistent feature. Guard the working `fetch` (no `--replies`) path — do not touch it.

The rest of the flags: `tweet --replies` → "include the reply/conversation thread" (plain — that this one works because TweetDetail needs no per-request token is §17 internal detail, not help-text material); `--wait-on-limit` → "on a 429, sleep until the rate-limit window resets instead of exiting 3"; `--max-wait` → "cap a --wait-on-limit sleep, in seconds (default: unbounded)"; `--by` → "how to read an all-digit identifier: numeric user id (default) or screen_name"; `--limit` → "stop after this many tweets (default: unbounded)"; `--since` → "keep tweets on/after this date YYYY-MM-DD; best-effort — if the run stops on --limit or the request budget before reaching it, exit 7 (see §11)"; `--until` → "keep tweets on/before this date YYYY-MM-DD"; `--format` → array vs NDJSON with default; `--output` → "(default: a timestamped file under the platform data dir, not cwd)"; `--profile` → "(default: 'default')"; `--profile-dir` → "(default: platform data dir, or $SFX_PROFILE_DIR)"; `--raw`, `-v`, `status --json` → same one-liner treatment as fb. Same formatter decision as fb: **explicit help strings, not `argparse.ArgumentDefaultsHelpFormatter`** — `--limit`/`--max-wait`/`--output`/`--profile-dir` default to `None`, which the formatter would render as a misleading bare "None". And the same boundary: `--help` carries CLI-intrinsic facts only; skill-context advice ("always pass `--output ./.tmp/x-fetch/…`", "never `--no-redact`") stays in the skill, because the CLI's own defaults are deliberately right for a standalone user.

**Part B — `scrape-x schema` (new subcommand).** Offline, deterministic, no session/network → always exit 0. Prints the output object schema so the skill never hand-copies the field list. X's shape is one level deeper than fb's: the array element is a `Tweet` whose `author` is a nested `User` (10 substantive keys) and whose `media` items are `Media` (5 keys) — so `schema` must describe **all three object types**, not just the top level; a bare `author : object` would leave the skill's most-asked question ("what's in author?") unanswered. Anchor everything on the OUTPUT contract, verified against the shipped model (2026-07-07): `Tweet.to_dict()` emits **24 keys normally, 25 with `--raw`**, while `dataclasses.fields(Tweet)` returns 25 including the conditionally-emitted `raw` — same divergence fb had, same fix: derive names from a representative `to_dict()`, mark `raw` present-only-with-`--raw`. `User.to_dict()` = 10 keys, `Media.to_dict()` = 5 keys, both unconditional. **A second, X-only reason `dataclasses.fields()` is the wrong anchor: `Media` has dataclass defaults (`width`/`height`/`alt_text` default `None`), so a naive "has-a-default ⇒ optional key" derivation would mis-document three always-present keys as optional** — `to_dict()` emits all 5 regardless. Types come from an explicit Python→JSON mapping, never the annotation: `created_at` `datetime | None` → `string | null` via `_iso()`, but `captured_at` `datetime` (non-optional — the tool always sets it to `datetime.now(UTC)`, `model.py:117`) → `string`, **always present, never null** — do not group the two, or `schema` will wrongly document `captured_at` as nullable (the shipped fb `schema` code caught this and documents its own `captured_at` as `string`, though the fb *plan* text has the same latent grouping error — don't copy the plan, copy the corrected code); `author` `User | None` → `object | null`; `media` `list[Media]` → `array<object>`; `urls`/`hashtags` `list[str]` → `array<string>`; `view_count` → `integer | null` (the raw payload carries it as a string; `_extract_view_count` converts — the JSON output type is what matters); `retweeted_tweet`/`quoted_tweet` `Tweet | None` → nested Tweet object or `null`. Descriptions live in dicts co-located with the dataclasses in `model.py` (one per class), same rationale as fb.

Output: human-readable annotated listing (all three objects) on **stdout**; `--json` emits JSON Schema draft 2020-12. Here X *should* go further than fb, but state the reason honestly: fb emitted a bare `{"type": "object"}` for **every** nested object — including the recursive `shared_post`, a full `Post` — as a deliberate simplification, not because those objects were trivial. X's nesting carries more that a consumer actually wants: `author` is a distinct `User` type (10 keys) that is the single most-asked question about the output, and `retweeted_tweet`/`quoted_tweet` are `Tweet`s. So for X, at minimum the **human listing must describe all three object types** (a bare "author : object" is useless); for `--json`, prefer real reusable definitions — `User`/`Media` as `$defs` with `Tweet` self-referencing for `retweeted_tweet`/`quoted_tweet` — but if a self-referencing draft-2020-12 `$ref` proves fiddly to get right, a documented bare `{"type": "object"}` (fb's own fallback) is acceptable *as long as the human listing is complete*. Don't let the JSON form block the release.

**What goes in the descriptions dict vs what stays in the skill — corrected against the shipped fb precedent.** The earlier draft claimed the field *usage* notes "can't be carried by `schema`, so they stay in the skill." That is wrong: fb's shipped `FIELD_DESCRIPTIONS` already carry exactly this kind of note — `id` says "dedupe on this, never on captured_at", `media` says the URL is "signed, expiring, viewer-scoped … treat as sensitive, never print unredacted", `captured_at` says "never a dedup key", and `shared_post` says "nesting isn't capped at one level". A field-attached gotcha belongs **in that field's description**, co-located in `model.py`, so it ships in `scrape-x schema` output for skill and direct user alike (more drift-free than a skill copy). So put these in the descriptions dict, each on its own field: dedup on `id` (and `captured_at` is *not* a dedup key); `media` URLs (`pbs.twimg.com`/`video.twimg.com`) are signed/expiring/sensitive; **a retweet's `text` is the retweeted ORIGINAL's text, not an "RT @…" stub** (on the `text` field — G-retweet-text); `retweeted_tweet`/`quoted_tweet` **can nest deeper than one level** (the serializer calls `build_tweet` recursively, so a quote-of-a-quote goes deeper — mirror fb's `shared_post` wording, and note the dataclass comment's "one level" describes intent, not what the code enforces); what `is_restricted` means; `is_pinned` tweets arrive out of chronological order. What genuinely **stays in the skill** (not a single field's description): the skill-context flag advice already carved out in Part A (`--output ./.tmp/x-fetch/…`, never `--no-redact`), and the cross-field *flag behavior* that isn't a property of any one field — that `--since`/`--until` skip `is_pinned` tweets and never compare a `created_at = None` tweet (§11), which is about how the date-window flags treat those fields, not about the fields themselves.

**Versioning & the new dependency.** v0.2.0, minor bump, CHANGELOG entry, existing CI/publish path unchanged (OIDC on GitHub Release, §15). The x-fetch skill is born requiring `scrape-x >= 0.2.0` (it points at `schema`, absent in 0.1.0): its readiness path must treat an unknown `schema` subcommand (argparse exit 1, "invalid choice") or a pre-0.2.0 `--version` as an upgrade signal, explicitly distinct from an exit-2 login problem — verified during fb's E2E re-test that a cold model diagnoses this correctly from one reference paragraph, so X-FETCH-PLAN carries the same paragraph (§3). **The upgrade command is `uv tool upgrade scraper-for-x`, NOT `uv tool install --upgrade scraper-for-x[...]`.** This matters here in a way it did not for fb: `scrape-x` has a `browser` extra (Path A installs `scraper-for-x[browser]`; Path B installs the base `scraper-for-x`), and `uv tool install --upgrade <spec>` re-resolves the tool from `<spec>` as written — so `--upgrade scraper-for-x` would strip the `[browser]` stack from a Path A user (breaking `login`/`setup`), while `--upgrade "scraper-for-x[browser]"` would force the browser stack onto a Path B user who deliberately went base-only. `uv tool upgrade scraper-for-x` upgrades in place from the recorded requirement, **preserving whichever extra was installed** — the single command correct for both paths. fb could safely copy `uv tool install --upgrade scraper-for-facebook` only because fb has no extras; do not carry that form over.

**Implementation order (next session, before any x-fetch work):** help strings incl. the dead-path markers, plus the dead-path exit-1 hoist (Part A), in `cli.py`; `FIELD_DESCRIPTIONS`-style dicts for `Tweet`/`User`/`Media` + `schema_fields()`/`json_schema()` beside `model.py`. Tests: (1) documented field sets equal `to_dict()` keys — Tweet checked both without `--raw` (24) and with (25), User (10), Media (5) — **not** `dataclasses.fields()` (25 for Tweet incl. `raw`; and Media's defaults would poison an optionality heuristic); (2) every `to_dict` key across all three classes has a description entry; (3) `captured_at` is documented `string` (non-null) while `created_at` is `string | null` — a regression guard for exactly the grouping error the fb plan carried; (4) **couple the dead-path help marker to the actual behavior** so the marker can't rot: assert that `search`'s help and `fetch`'s `--replies` help each contain "NOT IMPLEMENTED" *and* that invoking each still exits 1 — this is the drift surface the marker introduces (when a later version implements these ops and removes the raise, this test fails and forces the stale marker's removal, the same "turn drift into a caught error" discipline the field tests give the schema). Then: extend the wheel-install smoke test to run `schema`; bump + CHANGELOG; release per §15 (tag + GitHub Release → OIDC publish); verify on PyPI with a fresh-venv install AND upgrade any real local `uv tool` install (via `uv tool upgrade scraper-for-x`, extra-preserving) before the skill work starts — fb's E2E round caught a stale 0.1.0 local binary, so check first this time; then build x-fetch per X-FETCH-PLAN.
